<template>
	<a-card>
		<buttons :buttons="buttons" v-model="where" :searchMethodName="searchMethodName" :search="true" @input="_LoadData" />
		<page :current="pagination.current" :total="pagination.totalCount" :defaultPageSize="pagination.pageSize" @input="_LoadData"
		 v-model="pagination" />
		<a-table :columns="columns" :dataSource="dataList" :pagination="false" :loading="loading" :scroll="{x:1595,y:550}"
...
<script>
	...
	import {
		GenericColumns
	} from '@/utils/helper/ColumnsHelper'
		...
		methods: {
			...
			BJMX() {
				...
				if (this.selectedRows.length !== 1) {
					...
					this.$refs.DispatchWorkModalForm.show(this.selectedRows)
				}
			},
			// 关闭明细
			GBMX() {
