<template>
  <a-modal
    ...
    @cancel="onClose"
  >
    <a-form layout="inline" :form="form" @change="handleFormChange" :loading="loading" hideRequiredMark>
      <div class="container">
        <!-- <a-row>
        <a-col :span="12">-->
        <a-form-item :labelCol="{span:5}" label="员工编号" >
          <a-input
        ...
        <!-- </a-col>
        <a-col :span="12">-->
        <a-form-item :labelCol="{span:5}" label="员工姓名">
      ...
      <a-row>
        <a-col :span="12">-->
        <a-form-item :labelCol="{span:5}" label="性别" class="inputmargin-left">
          <a-switch
        ...
        <!-- </a-col>
        <a-col :span="12">-->
        <a-form-item :labelCol="{span:5}" label="所属部门" class="inputmargin-left">
          <a-tree-select
            v-decorator="['fDepartment',{rules: [{ required: true, message: '请选择所属部门' }]} ]"
            style="width: 174px"
      ...
      <a-row>
        <a-col :span="12">-->
        <a-form-item :labelCol="{span:5}" label="系统" class="inputmargin-left">
        ...
        <!-- </a-col>
        <a-col :span="12">-->
        <a-form-item :labelCol="{span:5}" label="上级主管" class="inputmargin-left">
          <a-tree-select
            style="width: 174px"
            :value="valueTree"
            :dropdownStyle="{ maxHeight: '400px', overflow: 'auto' }"
      ...
      <a-row v-if="FSystemUser">
        ...
        <template v-if="FSystemUser">
          ...
          <a-form-item :labelCol="{span:5}" label="员工密码">
            <a-input
              type="Password"
              v-decorator="['Password',{rules: [{ required: false, message: '请输入员工密码' }]}  ]"
              :disabled="IsEdit&&!!this.mdl.Password"
            ></a-input>
          </a-form-item>
          <a-form-item :labelCol="{span:5}" label="ID卡号">
            <a-input v-decorator="['fidCard',{rules:[{required:false}]}]"></a-input>
          </a-form-item>
        </template>
        <!-- </a-col>
      </a-row>
      <a-row>
        <a-col :span="12">-->
        <a-form-item :labelCol="{span:5}" label="手机号码">
          <a-input
            autocomplete="off"
            v-decorator="['FPhone',{rules: [{ required: false, message: '请输入手机号码' }]} ]"
          ></a-input>
        </a-form-item>
        <!-- </a-col>
        <a-col :span="12">-->
        ...
        <a-form-item :labelCol="{span:5}" label="邮件地址">
          <a-input
            autocomplete="off"
      ...
      <a-collapse defaultActiveKey="1">
        <a-collapse-panel header="ERP属性" style="background-color: #F2F2F2" key="1">
          <a-form-item label="对应ERP用户">
            <a-input v-decorator="['FERPUser',{rules: []} ]"></a-input>
          </a-form-item>
          <a-form-item label="对应ERP职员">
            <a-input v-decorator="['FERPOfficeClerk',{rules: []} ]"></a-input>
          </a-form-item>
        </a-collapse-panel>
      </a-collapse>
    </a-form>
...
export default {
  ...
  methods: {
    ...
    Edit(formData) {
      ...
      this.mdl.fidCard=formData.fidCard;
    ...
    handleSubmit() {
      ...
      if (this.IsEdit == false) {
        ...
        this.form.validateFields((err, values) => {
          values.fSex = this.Sex ? 1 : 2 
          values.fParentId = this.formData.fParentId 
          ...
          if (this.value != this.formData.fDepartment) {
            //判断如果改变部门
